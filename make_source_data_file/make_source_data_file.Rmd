...
pci_out_tab <- left_join(
  pci_raw, pci_fdr_df %>% 
    filter(dataset == "true") %>% 
    dplyr::select(representative, clustername, 
                  nObsRound, rssH0,  rssH1, F_statistic),
  by = c("representative", "clustername")) %>% 
  mutate(affected_by_carry_over = case_when(
    clustername %in% carry_over_cases ~ TRUE, TRUE ~ FALSE),
    tabilized_hit_at_10percent_FDR = case_when(
      representative %in% 
...
```{r}
brd_out_tab <- left_join(
  brd_raw, brd_fdr_df %>% filter(dataset == "true") %>%
    dplyr::select(representative, clustername, nObsRound,
                  rssH0,  rssH1, F_statistic),
  by = c("representative", "clustername")) %>% 
  mutate(affected_by_carry_over = case_when(
    ...
    stabilized_hit_at_10percent_FDR = case_when(
      representative %in% 
        filter(hdac8ii_hits_df, slopeH1 > 0)$representative ~ TRUE, 
      TRUE ~ FALSE),
    destabilized_hit_at_10percent_FDR = case_when(
      representative %in% 
        filter(hdac8ii_hits_df, slopeH1 < 0)$representative ~ TRUE, 
...
gtp_out_tab <- left_join(
  gtp_raw,  gtp_fdr_df %>% filter(dataset == "true") %>% 
    dplyr::select(representative, clustername, nObsRound,
                  rssH0, rssH1, F_statistic),
  by = c("representative", "clustername")) %>% 
  mutate(stabilized_hit_at_1percent_FDR = 
         ...
         stabilized_hit_at_10percent_FDR = 
           case_when(representative %in% 
                       filter(gtp_hits_10per, slopeH1 > 0)$representative ~ TRUE,
                     TRUE ~ FALSE),
         destabilized_hit_at_10percent_FDR = 
           case_when(representative %in% 
                       filter(gtp_hits_10per, slopeH1 < 0)$representative ~ TRUE,
         ...
         GO_annotated_ATP_binder = case_when(
           clustername %in% all_atp_binder$SYMBOL ~ TRUE, 
           TRUE ~ FALSE),
         GO_annotated_GTP_binder = case_when(
           clustername %in% all_gtp_binder$SYMBOL ~ TRUE, 
           TRUE ~ FALSE))
```
...
brd4_jq1_df <- filter(jq1_lys_df, clustername == "BRD4") %>% 
  dplyr::select(gene_name = clustername, qupm, qusm, 
                temperature, conc, log_conc, log2_value,
                rel_value)
```
...
```{r}
out_list <- c(
  "Figure2a" = all_alpha_fdr_df,
  "Figure2b" = b100_pr_df,
  "Figure3a/FigureS2a" = pano_out_tab,
  "Figure3b/FigureS2b" = jq1_out_tab,
  "Figure4b/FigureS4a" = pci_out_tab,
  "Figure4d/FigureS4b" = brd_out_tab,
  "Figure4e" = lap3_tab,
  "Figure5a" = gtp_out_tab,
  "Figure5b" = set_intersect_df,
  "FigureS1" = brd4_jq1_df,
  # "FigureS2a" = pano_out_tab,
  # "FigureS2b" = jq1_out_tab,
  "FigureS2c" = pano_venn_df,
  "FigureS2d" = jq1_venn_df,
  "FigureS3a-e" = examples_pano_df,
  "FigureS4c" = csk_pci_df
)
```

Write excel table
```{r}
write.xlsx(out_list, col.names=TRUE, row.names=TRUE, asTable = TRUE, 
